...
module "subscriptions" {
  ...
  resource_id      = each.key.subscription_key == "logged_in_subscription" ? format("/subscriptions/%s", local.client_config.subscription_id) : (each.key.subscription_key == "other_subscription" ? format("/subscriptions/%s", try(local.combined_objects_subscriptions[try(each.value.subscription.lz_key, local.client_config.landingzone_key)][each.value.subscription.key].subscription_id, null)) : format("/subscriptions/%s", each.value.subscription_id))
