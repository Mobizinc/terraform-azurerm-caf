resource "azurerm_security_center_automation" "automation" {
  ...
  dynamic "action" {
    ...
    resource_id       = action.value.type == "EventHub" ? var.event_hub[try(var.settings.lz_key, var.client_config)][action.value.resource_id].id : (action.value.type == "LogicApp" ? var.trigger_url[try(var.settings.lz_key, var.client_config)][action.value.resource_id].id : var.log_analytics[try(var.settings.lz_key, var.client_config)][action.value.resource_id].id)
    connection_string = action.value.type == "EventHub" ? try(var.event_hub_auth_rules[try(var.settings.lz_key, var.client_config)][action.value.resource_id].primary_connection_string, null) : null
