  ...
  dynamic "network_acls" {
    ...
    content {
      ...
      virtual_network_subnet_ids = try(var.settings.network.subnets, null) == null ? null : [
        for key, value in var.settings.network.subnets : try(var.vnets[var.client_config.landingzone_key][value.vnet_key].subnets[value.subnet_key].id, try(var.vnets[value.lz_key][value.vnet_key].subnets[value.subnet_key].id, value.remote_subnet_id))
      ]
    }
  }
