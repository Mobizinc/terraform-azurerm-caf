...
resource "azurerm_site_recovery_replication_policy" "policy" {
  ...
  managed_disk {
      disk_id                         = local.virtual_machine_os_disk_id
      staging_storage_account_id      = try(var.storage_accounts[try(each.value.managed_os_disk.storage_account.lz_key, var.client_config.landingzone_key)][each.value.managed_os_disk.storage_account.key].id, null)
      target_resource_group_id        = var.resource_groups[each.value.managed_os_disk.target_resource_group_key].id
      target_disk_type                = try(each.value.managed_os_disk.target_disk_type, null)
      target_replica_disk_type        = try(each.value.managed_os_disk.target_replica_disk_type, null)
      target_disk_encryption_set_id   = try(each.value.managed_os_disk.disk_encryption_set, null) == null ? null : var.disk_encryption_sets[try(each.value.managed_os_disk.disk_encryption_set.lz_key, var.client_config.landingzone_key)][each.value.managed_os_disk.disk_encryption_set.disk_encryption_set_key].id
  }
