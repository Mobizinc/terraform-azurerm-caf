...
resource "azurerm_app_service_slot_virtual_network_swift_connection" "vnet_config" {
 ...
 slot_name      = var.name
 app_service_id = try(local.combined_objects_app_services[try(each.value.app_services.lz_key, local.client_config.landingzone_key)][each.value.app_services.key].id, null)
