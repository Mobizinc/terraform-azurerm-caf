...
resource "azurerm_monitor_activity_log_alert" "mala" {
  ...
  scopes = try(flatten([
    for key, value in var.settings.scopes : coalesce(
      try(var.remote_objects[value.resource_type][var.client_config.landingzone_key][value.key].nics[value.nic_key].id, null),
  ...
  dynamic "criteria" {
    ...
    content {
      ...
      resource_id = try(
        var.remote_objects[criteria.value.resource.resource_type][var.client_config.landingzone_key][criteria.value.resource.key].nics[criteria.value.resource.nic_key].id,
