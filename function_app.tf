module "function_apps" {
  ...
  connection_strings         = try(each.value.connection_strings, tomap({ value = tomap({ name = "SQLAzure-ConnectionString", type = "SQLAzure", value = format("Server=tcp:%s,1433;Initial Catalog=%s;Persist Security Info=False;User ID=%s;Password=%s;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;", local.combined_objects_mssql_servers[try(each.value.lz_key, local.client_config.landingzone_key)][each.value.mssql_server_key].fully_qualified_domain_name, local.combined_objects_mssql_databases[try(each.value.lz_key, local.client_config.landingzone_key)][each.value.database_key].name, local.combined_objects_mssql_servers[try(each.value.lz_key, local.client_config.landingzone_key)][each.value.mssql_server_key].administrator_login, local.combined_objects_mssql_servers[try(each.value.lz_key, local.client_config.landingzone_key)][each.value.mssql_server_key].administrator_login_password) }) }), {})
